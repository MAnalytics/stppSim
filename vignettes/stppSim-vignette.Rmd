...
In the above code, the `s_range` parameter is used to set the spatial range. The default temporal bandwidth is 30 days with a daily incremental range. if `s_range` parameter is set as `NULL`, the function ignores the detection of space-time interaction and simply focuses on simulating the spatial and temporal patterns only. The NearRepeat calculator [see here](https://github.com/wsteenbeek/NearRepeat) can be utilized to examine the spatiotemporal interaction in any dataset as follows:
